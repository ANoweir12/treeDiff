...
const render = require('./chunks/render.cjs');
const morph = require('./chunks/morph.cjs');
require('./chunks/h.cjs');
require('./chunks/convert.cjs');
...
const navigate = async (url, selector, opts, goBack = false, scroll = 0) => {
  ...
  if (routeMap.has(url.pathname)) {
    ...
    if (route.vnode) {
      try {
        render.patch(currentEl, route.vnode);
      } catch (_err) {
        const el = route.html ? getEl(route.html.documentElement, selector) : render.createElement(route.vnode);
