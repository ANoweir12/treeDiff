...
const createElement = (vnode, attachField = true) => {
  ...
  if (typeof vnode === "object" && vnode?.flag === h.Flags.ENTITY) {
    if (vnode.el)
      return vnode.el;
    else
      return createElement(h.resolveVNode(vnode));
  }
  ...
  if (vnode.props) {
    for (const propName in vnode.props) {
      ...
      } else if (propName.charCodeAt(0) === h.X_CHAR) {
        if (propName.charCodeAt(3) === h.COLON_CHAR) {
          el.setAttributeNS(h.XML_NS, propName, String(propValue));
        } else if (propName.charCodeAt(5) === h.COLON_CHAR) {
          el.setAttributeNS(h.XLINK_NS, propName, String(propValue));
        }
  ...
  if (vnode.children) {
    if (vnode.flag === h.Flags.ELEMENT_TEXT_CHILDREN) {
...
const useChildren = (drivers = []) => (el, newVNode, oldVNode, commit = (work) => work(), effects = [], driver) => {
  ...
  if (!newVNodeChildren || newVNode.flag === h.Flags.ELEMENT_NO_CHILDREN) {
    if (!oldVNodeChildren)
      return finish(el);
    effects.push({
      el,
  ...
  if (newVNode.flag === h.Flags.ELEMENT_KEYED_CHILDREN) {
    if (!el[h.NODE_OBJECT_POOL_FIELD])
      el[h.NODE_OBJECT_POOL_FIELD] = /* @__PURE__ */ new Map();
    let oldHead = 0;
    let newHead = 0;
    ...
    if (oldHead > oldTail) {
      while (newHead <= newTail) {
        ...
        effects.push({
          el,
          type: h.EffectTypes.CREATE,
          flush: () => el.insertBefore(el[h.NODE_OBJECT_POOL_FIELD].get(newVNodeChildren[head].key) ?? createElement(newVNodeChildren[head], false), el.childNodes.item(head))
        });
      }
      ...
      while (oldHead <= oldTail) {
        ...
        el[h.NODE_OBJECT_POOL_FIELD].set(oldVNodeChildren[head].key, node);
        effects.push({
          el,
          type: h.EffectTypes.REMOVE,
      ...
      while (newHead <= newTail) {
        ...
        if (oldVNodePosition !== void 0) {
          ...
          effects.push({
            el,
            type: h.EffectTypes.CREATE,
            flush: () => el.insertBefore(el[h.NODE_OBJECT_POOL_FIELD].get(newVNodeChild.key) ?? createElement(newVNodeChild, false), el.childNodes.item(head))
          });
        }
      }
      for (const [oldVNodeKey, oldVNodeValue] of oldKeyMap) {
        const node = el.childNodes.item(oldVNodeValue);
        el[h.NODE_OBJECT_POOL_FIELD].set(oldVNodeKey, node);
        effects.push({
          el,
          type: h.EffectTypes.REMOVE,
          flush: () => el.removeChild(node)
  ...
  if (newVNode.flag === h.Flags.ELEMENT_TEXT_CHILDREN) {
    ...
    if (oldString !== newString) {
      effects.push({
        el,
        type: h.EffectTypes.REPLACE,
        flush: () => el.textContent = newString
      });
    }
    return finish(el);
  }
  if (newVNode.flag === void 0 || newVNode.flag === h.Flags.ELEMENT) {
    if (oldVNodeChildren && newVNodeChildren) {
      ...
      if (newVNodeChildren.length > oldVNodeChildren.length) {
        for (let i = commonLength; i < newVNodeChildren.length; ++i) {
          ...
          effects.push({ el, type: h.EffectTypes.CREATE, flush: () => el.appendChild(node) });
        }
...
const useNode = (drivers) => {
  const nodeDriver = (el, newVNode, oldVNode, commit = (work) => work(), effects = []) => {
    const resolvedOldVNode = h.resolveVNode(oldVNode) ?? el[h.OLD_VNODE_FIELD] ?? convert.fromDomNodeToVNode(el);
    const resolvedNewVNode = h.resolveVNode(newVNode);
    const finish = (element) => {
      return {
        el: element,
    ...
    if (resolvedNewVNode === void 0 || resolvedNewVNode === null) {
      ...
      if (!hasString && typeof resolvedOldVNode === "object" && typeof resolvedNewVNode === "object") {
        if (resolvedNewVNode.flag === h.Flags.ELEMENT_IGNORE || resolvedOldVNode.flag === h.Flags.ELEMENT_IGNORE) {
          return finish(el);
        }
        if (resolvedNewVNode.flag === h.Flags.ELEMENT_FORCE_UPDATE || resolvedOldVNode.flag === h.Flags.ELEMENT_FORCE_UPDATE) {
        ...
        if (resolvedOldVNode.key === void 0 && resolvedNewVNode.key === void 0 || resolvedOldVNode.key !== resolvedNewVNode?.key) {
          ...
          if (resolvedNewVNode.flag !== h.Flags.ELEMENT_SKIP_DRIVERS) {
            for (let i = 0; i < drivers.length; ++i) {
              commit(() => {
...
const updateProp = (el, propName, oldPropValue, newPropValue, effects) => {
  ...
  if (propName.startsWith("on")) {
    ...
    if (propName.charCodeAt(3) === h.COLON_CHAR) {
      el.setAttributeNS(h.XML_NS, propName, String(newPropValue));
    } else if (propName.charCodeAt(5) === h.COLON_CHAR) {
      el.setAttributeNS(h.XLINK_NS, propName, String(newPropValue));
    }
  } else if (el[propName] !== void 0 && !(el instanceof SVGElement)) {
    if (newPropValue) {
      effects.push({
      ...
      effects.push({
        el,
        type: h.EffectTypes.REMOVE_PROP,
        flush: () => {
          el[propName] = "";
...
const patch = (el, newVNode, oldVNode, hook = () => true, effects = []) => {
  ...
  effects.push({
    el: data.el,
    type: h.EffectTypes.SET_PROP,
    flush: () => data.el[h.OLD_VNODE_FIELD] = h.resolveVNode(newVNode)
  });
...
const render = (parentEl, newVNode, oldVNode, hook) => {
  const el = parentEl[h.DOM_REF_FIELD];
  if (el) {
    ...
    parentEl[h.DOM_REF_FIELD] = newEl;
    return newEl;
  }
};
